...
- Screenshot coverage now lives in the [`playwright` job of `ci.yml`](../.github/workflows/ci.yml). The `theme-previews` matrix entry runs `npx playwright test tests/e2e/gallery-preview.spec.ts … --project=chromium-w1440 --grep @visual` against the production build produced by the `build` job, capturing depth-aware snapshots and rerunning axe across `/preview/theme-matrix`.
- Need fresh images without waiting for a new commit? Re-run the `playwright` job from the GitHub Actions UI or execute `npx playwright test --config playwright.config.ts --project=chromium-w1440 --grep @visual` locally. Set `PLAYWRIGHT_ALLOW_LOCAL_FIRST=1` if you want the previews to exercise persistence scenarios that mutate local storage.
