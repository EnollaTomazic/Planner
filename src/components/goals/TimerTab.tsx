          ...
          <div className="relative mx-auto w-full max-w-sm rounded-2xl border border-card-hairline/60 bg-background/30 p-[var(--space-8)] backdrop-blur-xl shadow-card">
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 grid place-items-center">
                <div className="text-display font-semibold tabular-nums text-foreground drop-shadow-[0_0_8px_hsl(var(--neon-soft))] transition-transform duration-150 group-hover:translate-y-0.5 sm:text-display-lg">
                  {fmt(remaining)}
                </div>
                {isCustom && !running && (
                  <input
                    ...
                    onBlur={commitEdit}
                    onKeyDown={(e) => e.key === "Enter" && commitEdit()}
                    className="absolute w-full max-w-[7ch] rounded-full bg-transparent text-center text-display font-semibold tabular-nums opacity-0 focus:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:text-display-lg"
                  />
                )}
              </div>
            </div>
