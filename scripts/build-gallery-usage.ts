...
const PREVIEW_BACKGROUNDS = BG_CLASSES.map(
  (_, index) => index as Background,
) as readonly Background[];
...
async function buildGalleryManifest(
  ...
  const previewRoutesSource = JSON.stringify(previewRoutes, null, 2);
  const previewRoutesLiteralLines = previewRoutesSource
    .split("\n")
    .map((line) =>
  ...
  const lines = [
    ...
    "",
    "export const galleryPreviewRoutes = JSON.parse(`",
    ...previewRoutesLiteralLines,
    "`) as readonly GalleryPreviewRoute[];",
    "",
