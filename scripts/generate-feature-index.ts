...
function analyzeExports(file: string, content: string): ParsedExports {
  ...
  for (const statement of source.statements) {
    ...
    if (ts.isInterfaceDeclaration(statement) || ts.isTypeAliasDeclaration(statement)) {
      ...
      typeExports.add(statement.name.text);
      continue;
    }
...
async function buildExport(file: string): Promise<ExportInfo> {
  ...
  return {
    name: exports.hasDefault ? toExportName(file) : undefined,
...
async function generate(
  ...
  for (const file of sortedFiles) {
    ...
    bar.increment();
    const importPath =
      "./" + path.relative(dir, file).replace(/\\/g, "/").replace(/\.(tsx|ts)$/, "");
    ...
    if (valueExports.length > 0) {
      exports.push(`export { ${valueExports.join(", ")} } from "${importPath}";`);
    }
