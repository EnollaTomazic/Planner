  ...
  if (variant === "textarea") {
    ...
    control = (
      <>
        ...
        <ChevronDown
          aria-hidden
          className="pointer-events-none absolute right-[var(--space-4)] top-1/2 size-[var(--space-4)] -translate-y-1/2 text-muted-foreground transition-colors duration-200 ease-standard group-focus-within:text-accent-foreground"
        />
      </>
    );
          ...
          {options.map((option) => {
            ...
            return (
              <li key={option.value}>
                <div
                  className={cn(
                    "flex items-center justify-between gap-[var(--space-3)] rounded-[var(--radius-xl)] px-[var(--space-4)] py-[var(--space-3)] text-ui transition-colors duration-200 ease-standard",
                    selected
