...
### Regenerating gallery and theme assets

Generated gallery metadata and theme CSS are now ignored by Git. When you add new previews or tweak theme tokens, regenerate the artifacts locally before committing:
...
The `prebuild` hook (invoked automatically by `pnpm install`, `pnpm dev`, and `pnpm run build`) calls these scripts when inputs change, so CI reruns them automatically as well. Run them manually whenever you need to refresh gallery usage metrics or the compiled theme stylesheet outside the standard prebuild cycle.
