Planner is a collaborative product planning workspace that turns rough ideas into prioritized roadmaps. It layers AI-guided exploration on top of shared design tokens, feature specs, and release scheduling tools so distributed teams can align on scope, ship lists, and launch criteria in one place.
...
The dev script runs `pnpm run prebuild` to refresh autogenerated indexes before launching the local server.
